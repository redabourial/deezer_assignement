...
import { registerUser, fetchUser, extraReducers } from "./usersSlice";
...
describe('extraReducers', () => {
    ...
    it('registerUser.pending', () => {
        const [_,pendingCallback] = builder.addCase.mock.calls.find(c => c[0].type === "users/createUser/pending");
    ...
    it('registerUser.fulfilled', () => {
        const [_,fulfilledCallback] = builder.addCase.mock.calls.find(c => c[0].type === "users/createUser/fulfilled");
    ...
    it('registerUser.rejected', () => {
        const [_,rejectedCallback] = builder.addCase.mock.calls.find(c => c[0].type === "users/createUser/rejected");
    ...
    it('fetchUser.pending', () => {
        const [_,pendingCallback] = builder.addCase.mock.calls.find(c => c[0].type === "users/fetchUser/pending");
        pendingCallback(state);
        expect(state.error).toBe(null);
        expect(state.loading).toBe(true);
    });
    ...
    it('fetchUser.fulfilled', () => {
        const [_,fulfilledCallback] = builder.addCase.mock.calls.find(c => c[0].type === "users/fetchUser/fulfilled");
        const payload = { pk: 1, name: 'r' };
        fulfilledCallback(state, { payload });
        expect(state.loading).toBe(false);
        expect(state.data[payload.pk]).toEqual(payload);
    ...
    it('fetchUser.rejected', () => {
        const [_,rejectedCallback] = builder.addCase.mock.calls.find(c => c[0].type === "users/fetchUser/rejected");
        const error = { message: 'Error message' };
        rejectedCallback(state, { error });
        expect(state.loading).toBe(false);
        expect(state.error).toBe(error.message);
